...
export async function GET(
  ...
  try {
    ...
    console.log(`📡 /api/quran/verses/by_chapter/${chapterId}: Fetching verses with params:`, queryString);
    ...
    console.log(`📡 /api/quran/verses/by_chapter/${chapterId}: Request URL:`, url);
    ...
    if (!response.ok) {
      ...
      console.error(`❌ /api/quran/verses/by_chapter/${chapterId}: API request failed:`, {
    ...
    console.log(`✅ /api/quran/verses/by_chapter/${chapterId}: Successfully fetched`, data.verses?.length || 0, 'verses');
    
    // Log first verse for debugging
    if (data.verses && data.verses.length > 0) {
      const firstVerse = data.verses[0];
      console.log(`📖 /api/quran/verses/by_chapter/${chapterId}: First verse sample:`, {
        verse_key: firstVerse.verse_key,
        hasArabicText: !!firstVerse.text_uthmani,
        translationsCount: firstVerse.translations?.length || 0,
        translationResourceIds: firstVerse.translations?.map((t: any) => t.resource_id) || []
      });
      
      // Log translation details
      if (firstVerse.translations && firstVerse.translations.length > 0) {
        console.log(`🌐 /api/quran/verses/by_chapter/${chapterId}: Translation details:`, 
        ...
        console.warn(`⚠️ /api/quran/verses/by_chapter/${chapterId}: No translations in API response!`);
      }
    }
