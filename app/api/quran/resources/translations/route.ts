...
export async function GET(request: Request) {
  try {
    ...
    console.log('📡 /api/quran/resources/translations: Fetching available translations with params:', queryString);
    
    const token = await getAccessToken();
    const clientId = getClientId();
    const apiUrl = getApiUrl();
    
    const url = `${apiUrl}/resources/translations${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      ...
      console.error('❌ /api/quran/resources/translations: API request failed:', {
    ...
    console.log('✅ /api/quran/resources/translations: Successfully fetched', data.translations?.length || 0, 'translations');
